...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ apiKeyAvailable }) => {
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    setMessages(prevMessages => [
      ...prevMessages,
      {
        ...
        sender: Sender.AI,
        timestamp: new Date(),
        // groundingChunks: result.groundingChunks // Add grounding chunks if available
      },
    ]);
    ...
    for (let i = 0; i < fullAiText.length; i++) {
      ...
      }, i * 15); // Adjusted typing speed (15ms per character)
  ...
  const renderMessageText = (text: string): React.ReactNode => {
    ...
    // Regex to find links or bold markdown
    // Group 1: Full link | Group 2: Link text | Group 3: Link URL
    // Group 4: Full **bold** | Group 5: **bold** content
    // Group 6: Full __bold__ | Group 7: __bold__ content
    ...
    while ((match = markdownRegex.exec(text)) !== null) {
      ...
      if (match.index > lastIndex) {
        const plainTextPart = text.substring(lastIndex, match.index);
        plainTextPart.split(/(\n)/g).forEach(part => {
          if (part === '\n') {
            nodes.push(<br key={`br-text-${keyCounter++}`} />);
          } else if (part) {
            nodes.push(part);
          }
        });
      ...
      if (match[1]) { // Link: match[1] is full link, match[2] is text, match[3] is URL
        nodes.push(
          <a
            key={`link-${keyCounter++}`}
            href={match[3]}
      ...
      } else if (match[4]) { // **bold**: match[4] is full bold, match[5] is content
        const boldContent = match[5];
        const boldNodes: React.ReactNode[] = [];
        boldContent.split(/(\n)/g).forEach(part => {
          if (part === '\n') {
            boldNodes.push(<br key={`br-bold-star-${keyCounter}-${boldNodes.length}`} />);
          } else if (part) {
            boldNodes.push(part);
          }
        });
        nodes.push(<strong key={`bold-star-${keyCounter++}`}>{boldNodes}</strong>);
      } else if (match[6]) { // __bold__: match[6] is full bold, match[7] is content
        const boldContent = match[7];
        const boldNodes: React.ReactNode[] = [];
        boldContent.split(/(\n)/g).forEach(part => {
          if (part === '\n') {
            boldNodes.push(<br key={`br-bold-under-${keyCounter}-${boldNodes.length}`} />);
          } else if (part) {
            boldNodes.push(part);
          }
        });
        nodes.push(<strong key={`bold-under-${keyCounter++}`}>{boldNodes}</strong>);
      }
    ...
    if (lastIndex < text.length) {
      ...
      trailingText.split(/(\n)/g).forEach(part => {
        if (part === '\n') {
          nodes.push(<br key={`br-trail-${keyCounter++}`} />);
        } else if (part) {
          nodes.push(part);
        }
      });
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50 relative">
      ...
      {apiKeyAvailable && chatSession && (
        <div className="px-4 pt-4 pb-2 bg-white/50 border-t border-purple-100 flex flex-col items-center">
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          <p className="text-xs text-gray-500 text-center pt-2 pb-1 w-full max-w-2xl">
            © 2025 Ray Inc. for NCECII
          </p>
        </div>
      )}
    </div>
